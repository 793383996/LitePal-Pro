...
class UpdateSampleActivity : AppCompatActivity(), View.OnClickListener {
    ...
    private lateinit var nameToUpdateEdit: EditText
    private lateinit var ageToUpdateEdit: EditText
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        nameToUpdateEdit = findViewById(R.id.name_to_update)
        ageToUpdateEdit = findViewById(R.id.age_to_update)
        val updateBtn1 = findViewById<Button>(R.id.update_btn1)
        val updateBtn2 = findViewById<Button>(R.id.update_btn2)
        ...
        when (v.id) {
            R.id.update_btn1 -> {
                try {
                    ...
                    val singerName = singerNameEdit.text.toString()
                    val singerAge = singerAgeEdit.text.toString().toInt()
                    v.isEnabled = false
                    DbTaskExecutor.run(
                        task = {
                            val singer = Singer()
                            singer.name = singerName
                            singer.age = singerAge
                            singer.update(singerId)
                        },
            ...
            R.id.update_btn2 -> {
                try {
                    val singerName = singerNameEdit.text.toString()
                    val singerAge = singerAgeEdit.text.toString().toInt()
                    val targetName = nameToUpdateEdit.text.toString()
                    val targetAge = ageToUpdateEdit.text.toString()
                    v.isEnabled = false
                    DbTaskExecutor.run(
                        task = {
                            val singer = Singer()
    ...
    companion object {
        ...
        fun actionStart(context: Context) {
            val intent = Intent(context, UpdateSampleActivity::class.java)
